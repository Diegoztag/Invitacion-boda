...
class CreateConfirmationUseCase {
    ...
    async execute(invitationCode, confirmationData) {
        try {
            ...
            try {
                savedConfirmation = await this._createAndSaveConfirmation(
                    invitation,
                    normalizedData
                );
                ...
                // Rollback: Si se guardó la confirmación pero falló la actualización de la invitación
                if (savedConfirmation) {
                    this.logger.warn('Rolling back confirmation creation due to subsequent error', {
                        invitationCode,
                        error: innerError.message
                    });
                    try {
                        // Asumiendo que existe un método delete en el repositorio
                        if (typeof this.confirmationRepository.delete === 'function') {
...
module.exports = CreateConfirmationUseCase;
