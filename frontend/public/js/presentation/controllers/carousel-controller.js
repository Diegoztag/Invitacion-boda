...
export class CarouselController {
    ...
    async init() {
        ...
        console.log('🎠 Initializing CarouselController...');
    ...
    initializeCarousel() {
        ...
        // Cargar imágenes del primer slide
        ...
        console.log('🎠 Carousel initialized with slide 0');
    }
    ...
    async goToSlide(slideIndex) {
        if (this.isTransitioning || slideIndex === this.currentSlide || 
            slideIndex < 0 || slideIndex >= this.totalSlides) {
            return;
        }
        ...
        // Cargar imágenes del slide actual y siguiente
        this.loadSlideImages(slideIndex);
        if (slideIndex + 1 < this.totalSlides) {
            this.loadSlideImages(slideIndex + 1);
        }
        
        // Realizar transición
        await this.performTransition(previousSlide, slideIndex);
        ...
        this.emit(EVENTS.CAROUSEL.SLIDE_CHANGED, {
            from: previousSlide,
            to: slideIndex,
            slide: this.slideElements[slideIndex]
        });
    }
    
    /**
     * Realiza la transición entre slides
     * @param {number} fromIndex - Índice del slide anterior
     * @param {number} toIndex - Índice del slide nuevo
     * @returns {Promise}
     */
    performTransition(fromIndex, toIndex) {
        return new Promise((resolve) => {
            ...
            switch (this.options.animationType) {
                case 'fade':
                    this.performFadeTransition(fromSlide, toSlide, resolve);
                    break;
                    
                case 'scale':
                    this.performScaleTransition(fromSlide, toSlide, resolve);
                    break;
                    
                case 'slide':
                default:
                    this.performSlideTransition(fromSlide, toSlide, resolve);
                    break;
            }
        });
    }
    ...
    /**
     * Transición de slide
     * @param {HTMLElement} fromSlide - Slide anterior
     * @param {HTMLElement} toSlide - Slide nuevo
     * @param {Function} resolve - Callback de resolución
     */
    performSlideTransition(fromSlide, toSlide, resolve) {
        // Configurar posiciones iniciales
        toSlide.style.transform = 'translateX(100%)';
        toSlide.classList.add('active');
        
        // Animar
        requestAnimationFrame(() => {
            fromSlide.style.transition = `transform ${this.options.animationDuration}ms ease`;
            toSlide.style.transition = `transform ${this.options.animationDuration}ms ease`;
     ...
     * @param {Function} resolve - Callback de resolución
     */
    performFadeTransition(fromSlide, toSlide, resolve) {
        toSlide.style.opacity = '0';
        toSlide.classList.add('active');
        
        requestAnimationFrame(() => {
            fromSlide.style.transition = `opacity ${this.options.animationDuration}ms ease`;
            toSlide.style.transition = `opacity ${this.options.animationDuration}ms ease`;
     ...
     * @param {Function} resolve - Callback de resolución
     */
    performScaleTransition(fromSlide, toSlide, resolve) {
        toSlide.style.transform = 'scale(0.8)';
        toSlide.style.opacity = '0';
        toSlide.classList.add('active');
        
        requestAnimationFrame(() => {
            const transition = `transform ${this.options.animationDuration}ms ease, opacity ${this.options.animationDuration}ms ease`;
            fromSlide.style.transition = transition;
            toSlide.style.transition = transition;
    ...
    updateOptions(newOptions) {
        ...
        console.log('⚙️ Carousel options updated');
    }
    ...
    destroy() {
        ...
        this.isInitialized = false;
        console.log('🗑️ CarouselController destroyed');
    }
}
