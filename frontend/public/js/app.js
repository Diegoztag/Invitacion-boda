...
document.addEventListener('DOMContentLoaded', async () => {
    try {
        ...
        // Fallback a funcionalidad básica si falla la inicialización
        console.warn('🔄 Intentando cargar funcionalidad básica...');
        
        try {
            // Importar y usar el archivo legacy como fallback
            const { default: legacyApp } = await import('../app-legacy.js');
            if (typeof legacyApp === 'function') {
                legacyApp();
                console.log('✅ Funcionalidad básica cargada desde archivo legacy');
            }
        } catch (legacyError) {
            console.error('❌ Error al cargar funcionalidad básica:', legacyError);
            
            // Último recurso: mostrar mensaje de error al usuario
            showErrorMessage();
        }
    }
});
