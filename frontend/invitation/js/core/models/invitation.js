...
export class Invitation {
    constructor(data = {}) {
        ...
        if (dateStr && typeof dateStr === 'string' && dateStr.trim() !== '') {
            ...
            this.confirmedAt = dateStr;
        }

        // Lógica simplificada: Si hay fecha de confirmación válida, está confirmada
        if (this.confirmedAt) {
            this.confirmed = true;
            // Si hay pases confirmados (> 0), asiste. Si es 0, no asiste.
            this.attendance = this.guestCount > 0 ? 'yes' : 'no';
        } else {
            // Fallback a lógica anterior o datos directos
            if (data.status) {
                if (data.status === 'confirmed' || data.status === 'partial') {
                    this.confirmed = true;
                    this.attendance = 'yes';
                } else if (data.status === 'cancelled' && this.guestCount === 0) {
                    this.confirmed = true;
                    this.attendance = 'no';
                } else {
